...
| Pre-prepare raw block coalescing attempt | coalesce same-shard raw `TraceBlock`s into one larger block before `PreparedTraceBlockAppend::new`, then rerun fresh single and 5-round OTLP protobuf load on the same host | test whether cutting per-block prepare/encode calls materially improves end-to-end ingest | fresh single: official `401920.456 spans/s p99=0.655ms`, memory `347550.583 spans/s p99=0.642ms`, disk `420970.769 spans/s p99=0.433ms`; fresh 5-round median: official `393514.319 spans/s p99=0.686ms`, memory `200177.691 spans/s p99=1.146ms`, disk `407471.452 spans/s p99=0.488ms`; write throughput regressed, so this route was reverted | PASS |
| Stats-side bounded live-update drain verification | `cargo fmt --all`, `cargo test -p vtstorage -- --nocapture`, `cargo build --release --target aarch64-apple-darwin -p vtapi -p vtbench` after capping `stats()` to a bounded pending-update drain per call | verify `/metrics` no longer pays the full `drain_pending_trace_updates()` debt while small correctness tests stay green | formatting passed, full `vtstorage` tests passed, release build passed | PASS |
| OTLP protobuf load benchmark after keeping only bounded stats-side drain (fresh single run) | fresh clean start, same `vtbench otlp-protobuf-load` shape against official/memory/disk | confirm the read-side fix does not give back the recovered disk ingest lead | official `396475.630 spans/s p99=0.673ms`, memory `345892.289 spans/s p99=0.629ms`, disk `430192.512 spans/s p99=0.409ms` | PASS |
| OTLP protobuf load benchmark after keeping only bounded stats-side drain (fresh 5-round median) | fresh clean start, 5 serial rounds with the same official/memory/disk URLs | check that disk remains above official after reverting the regressing write-path experiment | official `343086.506 spans/s p99=0.902ms`, memory `135212.607 spans/s p99=2.698ms`, disk `359315.329 spans/s p99=0.713ms` | PASS |
| Post-run `/metrics` scrape after bounded stats-side drain | first `/metrics` scrape after the fresh 5-round disk run | measure whether the read-side debt actually moved from tens of seconds down to a tolerable bound | input/output blocks `1442528 -> 1416916`, max queue depth `6`, wait max `20310us`, first scrape dropped from about `30.7s` to about `14ms` | PASS |
