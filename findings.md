...
| Keep the disk-local combiner on the mainline, but feed it raw `TraceBlock`s before prepare | Queuing raw blocks and draining one more same-shard wave after the first prepare pass raises the physical append reduction to about `1.97%` (`1681824 -> 1648770`) and widens the lead over official without reintroducing the async worker / positive-wait regressions |
...
| The current disk mainline now has a meaningful buffer over official on this host, but not a giant one | Fresh single-run is now official `391557.761 spans/s` vs disk `430473.274`, and fresh 5-round median is official `390899.877` vs disk `423781.852`; that is strong enough to keep, but still leaves headroom if we can form materially bigger same-shard packets |
| Read-side `drain_pending_trace_updates()` is still the largest visible non-ingest debt | The first post-run `/metrics` scrape after the stronger disk run now took about `30.7s`, so the write path win is real but mixed-workload read visibility still needs separate cleanup |
